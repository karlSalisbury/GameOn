...
<?php 
	...
	{ 
	$_SESSION['error'] = 'Username taken. Please retry.'; //if an username is taken initialise a session called 'error' with a msg 
	header("location:registration.php"); //redirect to registration.php 
	exit(); 
	} 
	...
	$sql="INSERT INTO member (username, password, salt, firstName, lastName, street, suburb, state, postcode, country, phone, mobile, email, gender, joinDate, newsletter, type) VALUES ('$username', '$password', '$salt', '$firstName', '$lastName', '$street', '$suburb', '$state', '$postcode', '$country', '$phone', '$mobile', '$email', '$gender', NOW(), '$newsletter', '1')"; 
	}
	$result = mysqli_query($connect, $sql) or die(mysqli_error($connect)); //run the query 
